...
def path_key(path):
    """The cache-file name for `run-*.json`, `health-*.json` and `backlog-*.json`: the whole
    path, every `/` turned into `-`, nothing else touched and nothing cut off.

    **This name is an interface shared with Clawdline** — `ProjectStatus.key(forPath:)` there,
    `sed 's|/|-|g'` in a producer's shell — not an internal detail this reader may re-spell.

    So a space stays a space, because neither of the other two spellings replaces one, and `/`
    is the only character besides NUL that a name on this filesystem may not hold, which makes
    replacing it the whole of what a path needs to become a filename. Nothing is cut off
    either: any two project directories sharing their tail would land on one file, and one
    project's run or backlog drawn under another project's name is worse than an empty cell.
    """
    return path.replace("/", "-")
